...
<div class="flex items-center gap-2 mb-4">
                    ...
                    <x-button href="{{ route('students.import') }}" variant="secondary">
                        Import CSV
                    </x-button>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100">
                            ...
                            <th class="py-2 font-medium text-gray-600">Angkatan</th>
                    ...
                    <tbody>
                        @forelse ($students as $student)
                            <tr class="border-b border-gray-50">
                                ...
                                <td class="py-3 text-gray-800">{{ $student->user->email }}</td>
                                <td class="py-3 text-gray-800">{{ $student->nim }}</td>
                                <td class="py-3 text-gray-800">{{ $student->studyProgram->name }}</td>
                                <td class="py-3 text-gray-800">{{ $student->angkatan }}</td>
                                <td class="py-3">
                                    <div class="flex items-center gap-2">
                                        <x-button href="{{ route('students.edit', $student) }}" variant="primary" size="sm"
                                        ...
                                        <form action="{{ route('students.destroy', $student) }}" method="POST" onsubmit="return confirm('Yakin hapus mahasiswa ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-6 text-gray-400 text-center">Belum ada data mahasiswa.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
