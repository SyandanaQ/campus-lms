    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                ...
                <x-button href="{{ route('courses.create') }}" class="mb-4"
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100">
                            <th class="py-2 font-medium text-gray-600">Kode</th>
                            <th class="py-2 font-medium text-gray-600">Nama Mata Kuliah</th>
                    ...
                    <tbody>
                        @forelse ($courses as $course)
                            <tr class="border-b border-gray-50">
                                <td class="py-3 text-gray-800">{{ $course->code }}</td>
                                <td class="py-3 text-gray-800">{{ $course->name }}</td>
                                <td class="py-3 text-gray-800">{{ $course->sks }}</td>
                                <td class="py-3 text-gray-800">{{ $course->studyProgram->name }}</td>
                                <td class="py-3">
                                    <div class="flex items-center gap-2">
                                        <x-button href="{{ route('courses.edit', $course) }}" variant="primary" size="sm"
                                        ...
                                        <form action="{{ route('courses.destroy', $course) }}" method="POST" onsubmit="return confirm('Yakin hapus mata kuliah ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-6 text-gray-400 text-center">Belum ada data mata kuliah.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
