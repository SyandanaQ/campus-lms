    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                ...
                <x-button href="{{ route('study-programs.create') }}" class="mb-4"
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100">
                            <th class="py-2 font-medium text-gray-600">Nama Prodi</th>
                            <th class="py-2 font-medium text-gray-600">Fakultas</th>
                            <th class="py-2 font-medium text-gray-600">Jenjang</th>
                    ...
                    <tbody>
                        @forelse ($studyPrograms as $studyProgram)
                            <tr class="border-b border-gray-50">
                                <td class="py-3 text-gray-800">{{ $studyProgram->name }}</td>
                                <td class="py-3 text-gray-800">{{ $studyProgram->faculty->name }}</td>
                                <td class="py-3 text-gray-800">{{ $studyProgram->level }}</td>
                                <td class="py-3">
                                    <div class="flex items-center gap-2">
                                        <x-button href="{{ route('study-programs.edit', $studyProgram) }}" variant="primary" size="sm"
                                        ...
                                        <form action="{{ route('study-programs.destroy', $studyProgram) }}" method="POST" onsubmit="return confirm('Yakin hapus program studi ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-6 text-gray-400 text-center">Belum ada data program studi.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
